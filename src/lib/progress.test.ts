...
import { STORAGE_KEY, loadProgress, saveProgress } from './progress'
...
describe('STORAGE_KEY', () => {
  it('is the correct string constant', () => {
    expect(STORAGE_KEY).toBe('fp-ts-course:progress')
  })
})
...
describe('saveProgress()', () => {
  ...
  it('does nothing (no throw) when window is undefined (SSR guard)', () => {
    vi.stubGlobal('window', undefined)
    expect(() => saveProgress(new Set([1, 2]))).not.toThrow()
